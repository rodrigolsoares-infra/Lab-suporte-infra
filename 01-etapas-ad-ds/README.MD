...
Promoção do servidor a Controlador de Domínio (DC) da floresta `techcorp.local`. Foi estruturada uma hierarquia de Unidades Organizacionais (OUs) por departamento, criadas contas de usuários e grupos de segurança seguindo um modelo de controle de acesso (RBAC / AGDLP) e preparado o ambiente para integração híbrida com Microsoft Entra ID.
...
| ----------------------- | ---------------------------- | --------------------- | -------------------------- |
| **Domain Controller**   | Windows Server 2022 Standard | `DC01-CORP` (aka `SRV-DC01`) | `192.168.10.10 /24`        |
| **Gateway Virtual**     | vSwitch Interno (Hyper-V)    | `vSwitch-CORP-LAN` / `LAB-TECHCORP` | `192.168.10.1 /24`    |
...
| +-------------------------+             +-------------------------+ |
|  | DC01-SERVER (DC01-CORP)|  | CLI01-WIN11 |  |
|  | Windows Server 2022    |  | Windows 11 Pro |  |
|  | • AD DS (techcorp.local) | Conexão | • Ingressado no Domínio |  |
...
- Provisionamento da VM **Geração 2** (Suporte UEFI/Secure Boot) nomeada como `DC01-CORP` (também referenciado como `SRV-DC01` na documentação) com 4 GB de RAM dinâmica, 2 vCPUs e disco virtual de 60 GB.
...
<details>
<p align="center">
  <img src=".github/assets/01-hyperv-vswitch.png" alt="vSwitch e configs criadas no Hyper-V" width="700"><br>
  <i>Figura 1: Gerenciador de Comutador Virtual mostrando o `vSwitch-CORP-LAN` criado como rede Interna e as configurações da VM no Hyper-V mostrando a memória (4 GB) e vCPUs.</i>
</p>
</details>
...
#### 📸 Evidências Visuais da Promoção e Estrutura de OUs:
![Dominio Promovido](img/01-dominio-promovido.png)

### 2. Estrutura de OUs e Objetos de Usuários

- Estrutura de OUs por departamento foi criada (OU_TI, OU_Financeiro, OU_RH, OU_Vendas, OU_Diretoria, OU_Computadores, OU_Grupos) e grupos de segurança associados foram provisionados conforme RBAC/AGDLP.
...
<details>
<p align="center">
  <img src=".github/assets/05-adds-installation-ps.png" alt="Terminal do PowerShell executando o comando Install-ADDSForest." width="700"><br>
  <i>Figura 1: Terminal do PowerShell executando o comando `Install-ADDSForest` e a mensagem de reinício do sistema.</i>
</p>
<p align="center">
  <img src=".github/assets/06-domain-logon-screen.png" alt="Tela de logon do Windows Server exibindo o domínio" width="700"><br>
  <i>Figura 1: Tela de logon do Windows Server exibindo o domínio.</i>
</p>
<p align="center">
  <img src=".github/assets/07-dns-reverse-zone.png" alt="console do Gerenciador DNS mostrando a Zona de Pesquisa Inversa." width="700"><br>
...
<details>
<p align="center">
  <img src=".github/assets/09-aduc-ou-structure.png" alt="Print da janela do `dsa.msc` com a árvore do domínio `techcorp.local` expandida até a sub-OU `TI`, mostrando toda a hierarquia construída." width="700"><br>
...
<p align="center">
  <img src=".github/assets/11-upn-suffix-domains" alt="Print da janela de propriedades do _Domínios e Relações de Confiança do AD_ mostrando o sufixo `techcorp.com.br` adicionado." width="700"><br>
  <i>Figura 1: Print da janela do `dsa.msc` com a árvore do domínio `techcorp.local` expandida até a sub-OU `TI`, mostrando toda a hierarquia construída.</i>
</p>
<p align="center">
  <img src=".github/assets/12-user-upn-aduc.png" alt="Print das propriedades do usuário `Rodrigo Soares` no `dsa.msc` (aba _Conta / Account_) destacando o nome de logon com o sufixo `@techcorp.com.br`." width="700"><br>
...
<p align="center">
  <img src=".github/assets/13-agdlp-groups-members.png" alt="Print das propriedades do grupo `DL_Pasta_TI_RW` (aba _Membros_) mostrando o grupo `GRP_TI_ADM` aninhado." width="700"><br>
...
<details>
<p align="center">
  <img src=".github/assets/15-dhcp-scope-active.png" alt="Print do console do DHCP (`dhcpmgmt.msc`) mostrando o escopo `LAN_Corporativa` ativo com o pool de IPs configurado." width="700"><br>
...
<p align="center">
  <img src=".github/assets/16-file-server-folders.png" alt="Print do File Explorer no caminho `C:\Empresa` mostrando as subpastas dos setores (`TI`, `RH`, `Financeiro`, `Vendas`)." width="700"><br>
  <i>Figura 1: Configuração da rede virtual no Hyper-V.</i>
</p>
<p align="center">
  <img src=".github/assets/17-ntfs-permissions-ti.png" alt="Print da aba de **Segurança Avançada** da pasta `TI` mostrando a herança desabilitada e apenas o grupo `DL_Pasta_TI_RW` com permissão de Modificação." width="700"><br>
...
<details>
<p align="center">
  <img src=".github/assets/15-dhcp-scope-active.png" alt="Resultado do Diagnóstico dcdiag no Terminal." width="700"><br>
  <i>Figura 1: Resultado do Diagnóstico dcdiag no Terminal.</i>
</p>
<p align="center">
  <img src=".github/assets/16-file-server-folders.png" alt="Varredura de Identidades com a Ferramenta Microsoft IdFix." width="700"><br>
